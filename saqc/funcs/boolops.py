...
class BoolOpsMixin:
    def _booloperate(self, other, operator, s_rep=None, o_rep=None, o_kwargs=None):
        ...
        for f, of in zip(self.columns, other.columns):
            ...
            self._flags.history[f] = saqc.core.history.History(flagscol.index).append(
                flagscol, new_meta
            )
        return self
    ...
    def _arithmeticOperate(self, other, operator):
        ...
        if pd.api.types.is_scalar(other):
            ...
            other = saqc.SaQC(
                [
                    pd.Series(other, index=self.data[f].index, name=f)
            ...
            o_kwargs = [
                {"value": f"column: {k}", "type": str(other[k].dtype), "squeezed": True}
        ...
        for f, of in zip(self.columns, other.columns):
            out._data[f] = getattr(out.data[f], OPERATOR_FUNCS[operator])(
                other.data[of].values
            )
            ...
            if len(_meta) > 0:
                ...
                if len(ops) > 0:
                    unsqueezed = [
                        o
                        for o in ops
                        if not _meta[-1]["kwargs"][o].get("squeezed", False)
                    ]
                    if s_rep:
                        func_entry = f"|{f}|"
                        kwargs_entry = {
        ...
        for f, of in zip(self.columns, other.columns):
            if len(other._history[of].meta) > 0:
                for k in range(len(other._history[of].meta)):
                    self._flags.history[f].append(
                        other._history[of].hist.iloc[:, k],
                        other._flags.history[of].meta[k],
                    )
            else:
                self._flags.history[f].append(
                    other._flags[of],
                    {"func": "append", "args": (), "kwargs": {"field": of}},
                )
        if d_vals == "right":
    ...
    def __iand__(self, other):
        ...
        for f in self.columns:
            self._history[f].append(
                new_cols._flags[f], meta=new_cols._history[f].meta[-1]
            )
    ...
    def __itruediv__(self, other):
        return self.__lshift__(
            self.__truediv__(other, self_assign=True), d_vals="right"
        )
